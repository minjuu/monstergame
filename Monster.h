...
public:
  ...
  int& Map(vector<vector<int> >& map,int x, int y) { return map[y][x]; }
  ...
  void eat(vector<vector<int> > &map, bool isMonster = true)
  {
      ...
      //거리계산
      total += sqrt(pow(p.x - q.x, 2) + pow(p.y - q.y, 2));
      dist += sqrt(pow(p.x - q.x, 2) + pow(p.y - q.y, 2));
      //total += sqrt((double)(p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y));
      // += sqrt((double)(p.x - q.x) * (p.x - q.x) + (p.y - q.y) * (p.y - q.y));
      //휴식 여부
      if (dist > 20 && isMonster==true)
          nSleep += 10;
